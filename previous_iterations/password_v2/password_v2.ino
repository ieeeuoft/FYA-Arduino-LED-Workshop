...
const uint64_t questionMark = 0x0406b2b21e0c0000;    //yellow
...
void setup() {
    ...
    for(int i=0; i < 8; i++){   //needs to be yellow
        matrix.displaybuffer[i] = (questionMark & binaryAnd) >> (i*8);   //red portion
        matrix.displaybuffer[i] |= ((questionMark & binaryAnd) >> (i*8)) << 8; //green portion
...
void loop() {
    ...
    if(!buttonPressed){
        ...
        if(buttonPressed){
            ...
            //only clear if counter = 0 to preserve inputted code
            if (counter == 0){
                clearMatrix();
            ...
            switch(buttonState){
                case 1: //red button
                    ...
                    //Draw dot to display colourchosen
                    matrix.displaybuffer[0] |= (1 << (counter + 2));
                ...
                case 2: //yellow button
                    ...
                    //Draw dot to display colourchosen
                    matrix.displaybuffer[0] |= (1 << (counter + 2));
                    matrix.displaybuffer[0] |= (1 << (counter + 2)) << 8;
                    break;
